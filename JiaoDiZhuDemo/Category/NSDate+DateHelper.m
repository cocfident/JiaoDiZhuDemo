...
//日期格式化(根据当前时区进行转化)
-(NSString*)systemZoneStringWithFormat:(NSString*)format
{
    NSDateFormatter*outputFormatter =[NSDateFormatter sharedSystemZoneDateFormatter];
    [outputFormatter setDateFormat:format];
...
//东八区时间
+ (NSDate *)utc8DateWithInterval:(NSTimeInterval)intervalsecs
{
    NSDate *gmtDate = [NSDate dateWithTimeIntervalSince1970:intervalsecs];
    NSTimeZone *zone = [NSTimeZone timeZoneWithAbbreviation:@"GMT+0800"];
    NSInteger interval = [zone secondsFromGMTForDate:gmtDate];
    NSDate *utc8Date = [gmtDate dateByAddingTimeInterval:interval];
    return utc8Date;
    return gmtDate;
}
//字符串转换成日期（是什么就是什么）
...
//将一个日期转化成整点日期
+(NSDate*)integralHourFromDate:(NSDate *)date
{
    //整点时间
    NSNumber *year = [NSDate getYear:date];
    NSNumber *month = [NSDate getMonth:date];
    ...
    NSDate *integralDate = [NSDate localDateFromString:[NSString stringWithFormat:@"%@.%@.%@ %@",year,month,day,hour] withFormat:@"yyyy.MM.dd HH"];
...
//多少年前的今天
+ (NSDate *)TodayInHistory:(int)year
{
    NSDate *localDate = [NSDate localdate];
    int nowYear = [[NSDate getYear:localDate] intValue];
    int nowMonth = [[NSDate getMonth:localDate] intValue];
    int nowDay = [[NSDate getDay:localDate] intValue];
    
    NSDate *historyDate = [NSDate dateFromString:[NSString stringWithFormat:@"%d.%d.%d",(nowYear-year)<0?0:(nowYear-year),nowMonth,nowDay] withFormat:@"yyyy.MM.dd"];
    return historyDate;
}

//当前日期比另一个日期早多少天
- (NSInteger)earlierDaysDate:(NSDate *)anotherDate
{

    NSTimeInterval nowDateTimeInterval = [[NSDate integralDateFromDate:self] timeIntervalSince1970];
    NSTimeInterval anotherTimeInterval = [[NSDate integralDateFromDate:anotherDate] timeIntervalSince1970];
    
    return (nowDateTimeInterval - anotherTimeInterval)/(24*60*60);
